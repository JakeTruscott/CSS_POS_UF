---
title: "Distributed Representations of Words (Embeddings)"
...
### Distributed Representations of Words (Cont.)
\begin{itemize}
...
### Distributed Representations of Words -- Advantages
\begin{itemize}
...
### Estimating Embeddings -- Four Considerations (Rodriguez \& Spirling 2022)
\begin{enumerate}
...
```{r, imdb_data_load, include = F, eval = T}
set.seed(1234)
imdb <- bind_rows(get(load('../../class_7/data/imdb_train_pos.rdata')), 
                  get(load('../../class_7/data/imdb_train_neg.rdata'))) %>%
  dplyr::sample_n(1000)
...
### word2vec (Embeddings Themselves!)
\scriptsize
```{r, word2vec_3}
...
### word2vec (Visualize Embedding Space)
\scriptsize
```{r, word2vec_4}
...
### word2vec (Visualize Embedding Space - Cont.)

```{r word2vec_5, echo=FALSE, fig.height=2.75, fig.width=4.5}
...
data.frame(word = rownames(word_vec_subset),
  ...
  geom_text(vjust = -0.75, hjust = 0.5, size = 2.5) + 
...
words <- c('dog', 'cat', 'horse', 'pig', 'firetruck') # Words
...
### GloVe (Cont.)
\scriptsize
```{r glove_3, echo=FALSE, fig.height=2.75, fig.width=4.5}
...
### BERT -- Apply Using Python Code (Reticulate)
\scriptsize
```{r, bert_1}
...
### BERT -- Apply Using Python Code (Cont.)
\scriptsize
```{r, bert_2}
...
### BERT -- Same Application as Glove But Context-Specific Results! 
\scriptsize
```{r bert_4, echo=FALSE, fig.height=2.75, fig.width=4.5}
as.data.frame(cosine_sim_matrix) %>%
  mutate(Word1 = rownames(cosine_sim_matrix)) %>%
  tidyr::pivot_longer(cols = -Word1, names_to = "Word2", values_to = "CosineSim") %>%
  ggplot(aes(x = Word1, y = Word2, fill = CosineSim)) + 
...
### SBERT -- BERT Optimized for Sentences
\scriptsize
```{r sbert_1}
imdb <- imdb %>%
  dplyr::sample_n(1000) %>%
...
### SBERT -- BERT Optimized for Sentences (Cont.)
\scriptsize
```{r sbert_2}
