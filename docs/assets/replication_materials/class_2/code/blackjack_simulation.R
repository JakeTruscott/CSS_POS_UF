...
play_blackjack <- function(role = 'player', cards, interactive = TRUE) {
  ...
  repeat {
    ...
    if (is.numeric(hand) & role == 'dealer' & hand >= 17){
      cat('Dealer Stays at 17 or More! Final Hand Value: ', hand,  '\n')
...
data.frame(value = outcomes) %>%
  ...
  theme(
    ...
    legend.text = element_text(size = 14, colour = "black")) # 1,000 Figure

#########################################################
# How Many Blackjacks Over 1,000 Iterations? 5k? 10k?
#########################################################

seeds <- c(1234, 1235, 1236)
iterations <- c(1000, 5000, 10000)
...
for (n in 1:length(iterations)){
  ...
  outcomes <- character(iterations[n]) # Empty Character Vector (x1000)

  for (i in seq_len(iterations[n])) {

    if (i %% 100 == 0) {
      message("Completed ", i, " Games")
    }
  ...
  combined_outcomes[[as.character(iterations[n])]] <- outcomes # Export
...
purrr::imap_dfr(
  ...
  mutate(value = ifelse(value == 'Blackjack', 'Blackjack', 'Not Blackjack')) %>%
  group_by(run, value) %>%
  ...
  geom_col(position = position_dodge(), aes(fill = value), colour = 'black') +
  ...
  theme(
    ...
    legend.title = element_blank(),
    legend.background = element_rect(colour = "black", fill = NA),
    legend.text = element_text(size = 14, colour = "black")) # Combined Figure

# Proportions Hardly Change b/c Probability Didn't Change! Still ~4% chance of blackjack every hand
# Just Reduces Monte Carlo Variance!
...
seeds <- sample(c(1:10000), size= 1000, replace = F) # Draw Seeds from seed(1234)
days <- c(1:100)
combined_outcomes <- list()
...
for (day in 1:length(days)){
  ...
  for (i in seq_len(10)){ # At Most 10 Iterations
    ...
    if (i == 1){
      ...
      funds = outcomes[i - 1] # Else: Grab Funds
    }
...
total_funds <- sum(combined_outcomes$value) - 100000 # (Break-Even)
