...
\begin{itemize}
  ...
  \item Intuitive Analogy: 
  \begin{itemize}
    \item Imagine dumping all the words from a document into a bag -- shake it!
    \item Either reach into the bag and collect words or dump it out 
    \item Count the word frequencies 
  \end{itemize}
  \item Order and context are unnecessary (vector multiset) -- the existence (and prevalence) of words is sufficient to make sophisticated assessments about the document(s)
\end{itemize}
\end{frame}

\begin{frame}{The Bag of Words (Cont.)}
\phantomsection\label{the-bag-of-words-cont.}
\begin{itemize}
  \item \textbf{Why it's Useful}: Converts text into numerical features that can be used for classification, regression, and clustering tasks (\textit{Coming Soon!}). \par \vspace{2.5mm}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{validation }\OtherTok{\textless{}{-}} \FunctionTok{unlist}\NormalTok{(sotu\_eisenhower\_bush[[}\DecValTok{14}\NormalTok{]]}\SpecialCharTok{$}\NormalTok{military\_text)  }\CommentTok{\# Bush 1992 {-}{-} Print Example}

\FunctionTok{cat}\NormalTok{(validation)}
\end{Highlighting}
\end{Shaded}
...
gone, that process can be accelerated. Tonight I can tell you of
dramatic changes in our strategic nuclear force. These are actions we
...
missile. And we will not purchase any more advanced cruise missiles. The
Secretary of Defense recommended these cuts after consultation with the
Joint Chiefs of Staff. And I make them with confidence. But do not
misunderstand me. The reductions I have approved will save us an
...
\item \textbf{What are some additional terms we can use to capture rhetoric related to the \texttt{military}}?
\end{itemize}
\end{frame}
...
\item Develop a vocabulary and regular expression (regex) to capture parts of the speeches discussing those policy areas. 
...
\begin{frame}{Normalizing Text -- Big Consideration}
\phantomsection\label{normalizing-text-big-consideration}
\begin{itemize}
\item Denny \& Spirling (2018) -- \textbf{Main point}? \pause \par \vspace{2.5mm}
\item Pre-processing -- \textit{if \& how} -- can have fundamental impact on results. \par \vspace{2.5mm}
\item Week 4 Problem Set: How you partitioned text impacted your summary values \par \vspace{2.5mm}
